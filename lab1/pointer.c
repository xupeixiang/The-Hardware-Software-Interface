...
int withinSameBlock(int * ptr1, int * ptr2) {
  int mark = -1 << 6;
  return !(((int)ptr1 & mark) ^ ((int)ptr2 & mark));
}
...
int withinArray(int * intArray, int size, int * ptr) {
  int startInt = (int)intArray;
  int ptrInt = (int)(ptr);
  int delta = ptrInt - startInt;
  int big = 1 << 31;
  return !(((int)(intArray + size - 1) - ptrInt) & big) & !((ptrInt - startInt) & big)
      & ((delta & 3) == 0);
}
/*
